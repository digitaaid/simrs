<div id="penunjang">
    <x-adminlte-card theme="primary" title="Riwayat Penunjang Lainnya">
        @if (flash()->message)
        ...
        @endif
        <a href="#editPenunjang">
            <x-adminlte-button wire:click='tambahLab' class="btn-sm" label="Upload Penunjangg" theme="success"
            ...
            <tbody>
                @foreach ($rads as $item)
                    ...
                    @if ($lihat && $idLihat == $item->id)
                        <tr>
                            <td colspan="5">
                                <a href="{{ route('landingpage') . '/public/storage/penunjang/' . $item->filename }}"
                                    target="_blank">{{ route('landingpage') . '/public/storage/penunjang/' . $item->filename }}</a>
                                    <br>
                                <a href="{{ route('landingpage') . '/public/storage/penunjang/' . $item->filename }}" target="_blank">{{ $item->fileurl }}</a>
                                <iframe src="{{ route('landingpage') . '/public/storage/penunjang/' . $item->filename }}"
